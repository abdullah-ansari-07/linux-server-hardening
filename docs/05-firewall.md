...
# Introduction
Firewall is one of the most important defensive layer in securing a network. It controls network traffic server to and from the server. Network traffic should not have access to all the services in the server and must be restricted.
...
3. reject - block the connection and notify the sender
4. limit  - rate limit the connections attempts
- Port 22
SSH uses TCP port 22. It must remain open before enabling the firewall because the firewall rules are not configured yet and can block us out of the server as well. There is no other method to access the instance after getting blocked from our own server.
...
```
Check the status of UFW and allow OpenSSH before enabling firewall as not doing so will block the user out of the server.
